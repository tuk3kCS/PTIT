//[Mã câu hỏi (qCode): 8tjvKJD1].  Một chương trình (tạm gọi là RMI Server) cung cấp giao diện cho phép triệu gọi từ xa để xử lý dữ liệu nhị phân.
//Giao diện từ xa:
...
public class tjvKJD1 {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < data.length; i++) {
            encoded[i] = (byte) ((data[i] + shift) % 256);
        }

        System.out.println("Result: " + new String(encoded));

        service.submitData("B22DCDT294", "8tjvKJD1", encoded);
    }
}
